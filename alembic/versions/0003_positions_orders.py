...
def upgrade():
    ...
    if "investment_orders" not in tables:
...
def downgrade():
    insp=sa.inspect(op.get_bind())
    tables=set(insp.get_table_names())
    if "investment_orders" in tables: op.drop_table("investment_orders")
    if "portfolio_positions" in tables: op.drop_table("portfolio_positions")
