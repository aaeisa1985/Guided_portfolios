...
from app.models import Instrument, InvestmentOrder, ExecutionFill, PortfolioPosition, LedgerJournal, LedgerEntry
...
def apply_execution(session:Session, order:InvestmentOrder, execution_id:str, quantity:Decimal, price:Decimal, fees:Decimal=Decimal("0"), executed_at=None):
    if quantity<=0 or price<=0 or fees<0:
        raise ValueError("Execution quantity and price must be positive; fees cannot be negative")
    locked_order=session.scalar(select(InvestmentOrder).where(InvestmentOrder.id==order.id).with_for_update())
    ...
    if executed+quantity>order.quantity:
        raise ValueError("Execution exceeds remaining order quantity")
    ...
    if not position:
        position=PortfolioPosition(account_id=order.account_id,portfolio_id=order.portfolio_id,instrument_id=order.instrument_id,currency=instrument.currency,price_source="EXECUTION",price_as_of=now)
    ...
    if order.side=="BUY":
        ...
        position.price_as_of=now
        position.price_source="EXECUTION"
        ...
        if quantity>position.quantity:
            raise ValueError("Sell execution exceeds available position")
    ...
    fill=ExecutionFill(order_id=order.id,execution_id=execution_id,quantity=quantity,price=price,fees=fees,executed_at=now)
    session.add(fill)
