...
async function subscriptions(){
 ...
 return '<div class="section-title">Subscriptions</div><h2>Your subscriptions</h2><div class="card">'+(xs.length?xs.map(s=>'<div class="subscription-row"><div><b>'+esc(String(s.portfolio_id))+'</b><div class="hint">'+fmtDate(s.created_at)+'</div></div><div class="subscription-value">'+money(s.subscription_amount)+'<br>'+chip(s.status)+'</div></div>').join(''):'<div class="empty"><div class="empty-icon">◌</div><h3>No subscriptions yet</h3><p class="hint">Explore the portfolio shelf to start your first guided investment.</p><button class="btn btn-primary" onclick="nav(\'portfolios\')">Explore portfolios</button></div>')+'</div>';
}
function activity(){return '<div class="section-title">Activity</div><h2>Your activity log</h2><div class="card"><div class="timeline"><div class="timeline-item"><span class="timeline-dot"></span><div><b>Account activity is recorded</b><p class="hint">Suitability checks, consents and subscription events are retained by the backend audit trail.</p></div></div></div></div>'}
