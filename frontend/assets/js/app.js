...
function managerTab(tab){state.managerTab=tab;renderManagerTab(tab)}
...
function managerRenderEditor(d){
 ...
 '<div class="card"><div class="section-title">Version governance</div><p class="hint">Create a draft version before a controlled release.</p><button class="btn btn-ghost btn-small" '+(p.id?'':'disabled')+' onclick="managerCreateVersion(\''+(p.id||'')+'\')">Create draft version</button></div>'+
 ...
 managerHydrateInstrumentSelectors();managerUpdateTotals();
}
...
async function managerCreateVersion(id){try{await managerReq('/manager/portfolios/'+id+'/versions',{method:'POST'});toast('Draft version created','success')}catch(e){toast(e.message,'error')}}
